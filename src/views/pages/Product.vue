<template>
  <PageWrapper title="Produtos">
    <div class="mb-4 border-b border-gray-200 dark:border-gray-700">
      <ul
        class="flex flex-wrap -mb-px text-sm font-medium
         text-center text-gray-500 dark:text-gray-400"
        id="tabExample"
        role="tablist"
      >
        <li class="mr-2" role="presentation">
          <button
            class="
              inline-flex
              p-4
              rounded-t-lg
              border-b-2 border-transparent
              hover:text-gray-600 hover:border-gray-300
              dark:hover:text-gray-300
              group
            "
            id="product-tab-"
            type="button"
            role="tab"
            aria-controls="product-tab"
            aria-selected="false"
          >
            <InboxIcon class="flex-shrink-0 w-6 h-5" aria-hidden="true"/>
            <span class="ml-1">Produto</span>
          </button>
        </li>
        <li class="mr-2" role="presentation">
          <button
            class="
              inline-flex
              p-4
              rounded-t-lg
              border-b-2 border-transparent
              hover:text-gray-600 hover:border-gray-300
              dark:hover:text-gray-300
              group
            "
            id="estoque-tab-"
            type="button"
            role="tab"
            aria-controls="estoque-tab"
            aria-selected="false"
          >
            <InboxInIcon class="flex-shrink-0 w-6 h-5" aria-hidden="true" />
            <span class="ml-1">Estoque</span> 
          </button>
        </li>
       
      </ul>
    </div>
    <div id="tabContentExample">
      <!--  -->
      <ProdutcIndex 
        :load="loadProduct"
      />
      <Estoque 
        :load="loadEstoque"
      />
    </div>
  </PageWrapper>
</template>

<script>
import ProdutcIndex from '@/components/pages/product/Index.vue'
import Estoque from '@/components/pages/product/Estoque.vue'

export default {
    components: {
        ProdutcIndex,
        Estoque
    },
    data(){
        return {
            loadProduct: true,
            loadEstoque: false
        }
    },
  mounted() {
    const tabElements = [
      {
        id: 'product',
        triggerEl: document.querySelector('#product-tab-'),
        targetEl: document.querySelector('#product-tab'),
      },
      {
        id: 'estoque',
        triggerEl: document.querySelector('#estoque-tab-'),
        targetEl: document.querySelector('#estoque-tab'),
      }
    ]

    // options with default values
    const options = {
      defaultTabId: 'product',
      activeClasses:
        'text-blue-600 hover:text-blue-600 dark:text-blue-500 dark:hover:text-blue-400 border-blue-600 dark:border-blue-500',
      inactiveClasses:
        'text-gray-500 hover:text-gray-600 dark:text-gray-400 border-gray-100 hover:border-gray-300 dark:border-gray-700 dark:hover:text-gray-300',
      onShow: async (e) => {
        // console.log('tab is shown', e)
        switch (e._activeTab.id) {
          case 'product':
            return (this.loadProduct = true)
        //   case 'estoque':
        //     return this.loadEstoque = true
        }
        // this.loadUsers = false
        // this.loadPermission = false
      },
    }
 
    const tabs = new Tabs(tabElements, options)

    tabs.show('product')
    tabs.getActiveTab()

  }

}
</script>

<script setup>
import PageWrapper from '@/components/PageWrapper.vue'
import { TagIcon, InboxInIcon, InboxIcon, ShoppingCartIcon } from '@heroicons/vue/outline'

</script>
